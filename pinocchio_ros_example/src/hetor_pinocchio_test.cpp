...
// PINOCCHIO_MODEL_DIR is defined by the CMake but you can define your own directory here.
  ...
  #define PINOCCHIO_MODEL_DIR "/home/cherish/catkin_ws/src"
#endif
int main(int argc, char **argv)
{
  ros::init(argc, argv, "Pinocchio_hector_test");
  ros::NodeHandle nh;
  using namespace pinocchio;

  // You should change here to set up your own URDF file or just pass it as an argument of this example.
  const std::string urdf_filename = (argc<=1) ? PINOCCHIO_MODEL_DIR + std::string("/human_robot_hector/urdf/human_hector.urdf") : argv[1];
...
//  Eigen::VectorXd q= randomConfiguration(model);
  ...
  Eigen::Matrix<double,6,10> J1,J2,dJ;//the model has 10 joints
...
//   dJ=J1;
  ...
  std::cout << "model.njoints: " << model.njoints << std::endl;
    ...
    // Print out the information of each joint
    // for (size_t i = 0; i < model.njoints; ++i) {
    //     std::cout << "Joint " << i << " name: " << model.names[i] << std::endl;
    //     std::cout << "Joint " << i << " type: " << model.joints[i].shortname()<< std::endl;
    //     std::cout << "Joint " << i << " parent index: " << model.parents[i] << std::endl;
    //     // std::cout << "Joint " << i << " model.getJointId: " << model.getJointId("L_hip_joint")<< std::endl;
    //     std::cout << "-------------------------" << std::endl;
    // }
...
//* Perform the forward kinematics over the kinematic tree *
  ...
  for(JointIndex joint_id = 0; joint_id < (JointIndex)model.njoints; ++joint_id)
  {
    std::cout << std::setw(24) << std::left
              ...
              <<"------"<<std::endl
              << data.liMi[joint_id].rotation()<<std::endl
              <<"------"<<std::endl               /*Matrix : rotation transform matrix of the joint (relative to the body parent)*/
